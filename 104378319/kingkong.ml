...
let rec exec (t : term) : string = 
  match t with 
  | Name s -> " Push " ^ s ^ " Lookup "

  | Int i -> " Push " ^ string_of_int i 

  | Unit ->  " Push () "

  | Add (h, t) -> (exec h) ^ (exec t) ^ " Add "

  | Sub (h, t) -> (exec h) ^ (exec t) ^ " Sub "

  | Mul (h, t) -> (exec h) ^  (exec t) ^ " Mul "

  | Div (h, t) ->  (exec h) ^ (exec t) ^ " Div "

  | Trace s -> exec s ^ " Trace "

  | Ifgz (la1, la2, la3) ->  (exec la1) ^ " If " ^ " Begin " ^ (exec la2) ^ " End " ^ " Else " ^ " Begin " ^ (exec la3) ^ " End " ^ " End "

  | LetIn (la4, la5, la6) -> " Begin " ^ " Push " ^ la4 ^ (exec la5) ^ " Let " ^ (exec la6) ^ " End "

  | Fun (a34, b35, c36) -> " Fun " ^ a34 ^ " " ^ b35 ^ (exec c36) ^ " End " ^ " Push " ^ a34 ^ " Lookup "

  | App (q, z1) -> exec q  ^ exec z1  ^ " Call "
